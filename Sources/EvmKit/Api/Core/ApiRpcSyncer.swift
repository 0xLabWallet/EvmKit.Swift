...
class ApiRpcSyncer {
    ...
    init(rpcApiProvider: IRpcApiProvider, reachabilityManager: ReachabilityManager, syncInterval: TimeInterval) {
        ...
        NotificationCenter.default.addObserver(
            self, selector: #selector(onEnterBackground), name: UIApplication.didEnterBackgroundNotification,
            object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(onEnterForeground), name: UIApplication.willEnterForegroundNotification,
            object: nil)
    }
    ...
    @objc func onEnterForeground() {
        ...
        startTimer()
        // Обновляем lastBlockHeight при возврате из фона
        syncLastBlockHeight()
    }
    ...
    private func handleUpdate(reachable: Bool) {
        ...
        if reachable {
            state = .ready
            startTimer()
            // Сразу обновляем lastBlockHeight при старте, не ждем таймер
            syncLastBlockHeight()
        } else {
